...
if(isset($_GET['id'])){
    ...
    if(!$row){
        echo '<div style="text-align:center; padding:50px;"><h3>Order not found!</h3></div>';
        exit;
    }
...
<head>
    ...
    <style>
        body { background-color: #f3f4f6; padding: 30px 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        .invoice-container { background: #fff; max-width: 850px; margin: auto; padding: 20px; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); position: relative; }
        .invoice-header { border-bottom: 2px solid #eee; padding-bottom: 20px; margin-bottom: 30px; }
        .company-logo { font-size: 28px; font-weight: 800; color: #198754; letter-spacing: 1px; }
        ...
        .client-info h6, .company-info h6 { font-weight: 700; color: #444; text-transform: uppercase; font-size: 0.85rem; margin-bottom: 10px; }
        .table-invoice th { background-color: #f8f9fa; font-weight: 600; text-transform: uppercase; font-size: 0.8rem; color: #555; border-bottom: 2px solid #e9ecef; }
        .table-invoice td { vertical-align: middle; font-size: 0.95rem; padding: 12px 10px; }
        .total-section { background-color: #f8f9fa; padding: 20px; border-radius: 6px; }
        .grand-total { font-size: 1.2rem; font-weight: bold; color: #198754; }
        .footer-note { margin-top: 20px; padding-top: 20px; border-top: 1px solid #eee; color: #888; font-size: 0.85rem; text-align: center; }
        @media print {
...
<body>
    ...
    <div class="container text-center mb-4 no-print">
        <button onclick="downloadPDF()" class="btn btn-primary px-4 shadow-sm">
            <i class="bi bi-printer-fill me-2"></i> Download Invoice
        </button>
